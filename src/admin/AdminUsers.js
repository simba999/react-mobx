...
class AdminUsers extends Component {
	...
	csvEsc(str){
		str = str.trim();
		str = str.substring(0,20000); // Truncating string length to accommodate Intercom custom attribute character limit
		str.replace( /[\r\n]/g, ' ' );
	...
	componentWillMount(){
			...
			fire.database().ref('/users').on('child_added', s => {
				...
				let props = 'phone,dob,company,title,role,technology,bio,app,bio,brand,experience,goal,industry,interests,lookingForMentor,superpower,obsessions,topics';
				props = props.split(',');
